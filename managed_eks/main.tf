...
resource "aws_security_group" "eks_security_group" {
  ...
  description = "Security group for EKS with SSH and Vault access"
  ...
  ingress {
    ...
    to_port     = 8200
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"] # Replace with specific IP ranges for security if needed
    description = "Vault port for HTTP API access"
  }
...
resource "aws_iam_openid_connect_provider" "eks_oidc" {
  ...
  thumbprint_list = ["9e99a48a9960b14926bb7f3b3649e3d9e7c80e9b"]
...
# Define an output for the OIDC provider thumbprint
output "cluster_oidc_thumbprint" {
  value = aws_iam_openid_connect_provider.eks_oidc.thumbprint_list[0]
}
